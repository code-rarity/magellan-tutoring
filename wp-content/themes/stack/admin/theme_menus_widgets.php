	...
	function ebor_register_nav_menus() {
		register_nav_menus( 
			array(
				...
				'centered-inline-right'  => esc_html__( 'Centered, Inline Logo Right Navigation', 'stack' )
			) 
		);
	}
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( strcasecmp( $item->attr_title, 'divider' ) == 0 && $depth === 1 ) {
				...
				if( $item->object == 'mega_menu' ) {
					...
					if(!( '' == get_post_meta($item->object_id, '_ebor_menu_show_image', 1) )){
						$show_image = get_post_meta($item->object_id, '_ebor_menu_show_image', 1);	
					}
					...
					if( !( 'dropdowns--click' == $dropdown_method ) && $url = get_post_meta($item->object_id, '_ebor_top_level_url', 1) ){
						...
						$output .= '<a href="'. esc_url($url) .'" class="dropdown__trigger" target="'. $item->target .'"><span>'. $item->title .'</span></a>';
					...
					$output .= '<div class="dropdown__container"><div class="container"><div class="row">
							<div class="dropdown__content dropdown__content--lg col-sm-'. $dropdown_class .' '. $show_image .'">
								<div class="pos-absolute col-md-5 imagebg hidden-sm hidden-xs" data-overlay="4">';

									if ( has_post_thumbnail($item->object_id) ) {
									    $output .= '<div class="background-image-holder">'. get_the_post_thumbnail($item->object_id) .'</div>';
									}									

						$output .= '<div class="container pos-vertical-center">
										<div class="row">
					...
					if(!( '' == get_post_meta($item->object_id, '_ebor_menu_1', 1) )){
                        ...
                        foreach( $menu_post as $menu_item ){
                        	...
                        	$before = ( '#' == $menu_item->url ) ? '<h5>' : '<a href="'. $menu_item->url .'" '. $target .'>';
					...
					$output .= '</div><!--end dropdown content--></div></div></div>';
